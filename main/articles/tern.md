...
\
`adsl`` ``<-`` ``formatters``::`[`ex_adsl`](https://pharmaverse.github.io/formatters/latest-tag/reference/cdisc_data.html)\
`adlb`` ``<-`` ``formatters``::`[`ex_adlb`](https://pharmaverse.github.io/formatters/latest-tag/reference/cdisc_data.html)\
`adlb`` ``<-`` ``dplyr``::`[`filter`](https://dplyr.tidyverse.org/reference/filter.html)`(``adlb``, ``PARAMCD`` ``==`` ``"ALT"``, ``AVISIT`` ``!=`` ``"SCREENING"``)`
...
\
`# Mean with CI`\
[`g_lineplot`](https://pharmaverse.github.io/tern/reference/g_lineplot.md)`(``adlb``, ``adsl``, subtitle ``=`` ``"Laboratory Test:"``)`
...
`  ``adlb``,`\
`  ``adsl``,`\
`  table ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``"n"``, ``"mean"``, ``"mean_ci"``)``,`\
