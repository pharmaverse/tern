...
`adsl`` ``<-`` ``formatters``::`[`ex_adsl`](https://pharmaverse.github.io/formatters/latest-tag/reference/cdisc_data.html)` ``adlb`` ``<-`` ``formatters``::`[`ex_adlb`](https://pharmaverse.github.io/formatters/latest-tag/reference/cdisc_data.html)` ``adlb`` ``<-`` ``dplyr``::`[`filter`](https://dplyr.tidyverse.org/reference/filter.html)`(``adlb``, ``PARAMCD`` ``==`` ``"ALT"``, ``AVISIT`` ``!=`` ``"SCREENING"``)`
