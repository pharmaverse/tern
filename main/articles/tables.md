...
`adsl``$``ARM`` ``<-`` `[`factor`](https://rdrr.io/r/base/factor.html)`(``adsl``$``ARM``, levels ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``"B: Placebo"``, ``"A: Drug X"``, ``"C: Combination"``)``)`\
\
...
\
[`s_num_patients`](https://pharmaverse.github.io/tern/reference/summarize_num_patients.md)`(``x ``=`` ``adae``$``USUBJID``, labelstr ``=`` ``""``, .N_col ``=`` `[`nrow`](https://rdrr.io/r/base/nrow.html)`(``adae``)``)`\
`#> $unique`\
...
`anl`` ``<-`` ``adrs`` ``|>`\
`  ``dplyr``::`[`filter`](https://dplyr.tidyverse.org/reference/filter.html)`(``PARAMCD`` ``==`` ``"BESRSPI"``)`` ``|>`\
`  ``dplyr``::`[`mutate`](https://dplyr.tidyverse.org/reference/mutate.html)`(``is_rsp ``=`` ``AVALC`` `[`%in%`](https://rdrr.io/r/base/match.html)` `[`c`](https://rdrr.io/r/base/c.html)`(``"CR"``, ``"PR"``)``)`
